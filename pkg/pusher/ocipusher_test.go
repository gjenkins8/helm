...
func TestNewOCIPusher(t *testing.T) {
	...
	if op.opts.certFile != pub {
		assert.Equal(t, pub, op.opts.certFile)
	}

	if op.opts.keyFile != priv {
		assert.Equal(t, priv, op.opts.keyFile)
	}

	if op.opts.caFile != ca {
		assert.Equal(t, ca, op.opts.caFile)
	}

	if op.opts.plainHTTP != plainHTTP {
		assert.Equal(t, plainHTTP, op.opts.plainHTTP)
	}

	if op.opts.insecureSkipTLSVerify != insecureSkipTLSVerify {
		assert.Equal(t, insecureSkipTLSVerify, op.opts.insecureSkipTLSVerify)
	}
	...
	if op.opts.registryClient != registryClient {
		assert.Equal(t, registryClient, op.opts.registryClient)
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				...
				if client == nil {
					require.NotNil(t, op.opts.registryClient, "Expected non-nil registry client")
				}
			}
		})
	}
...
func TestOCIPusher_Push_MultipleOptions(t *testing.T) {
	...
	if err == nil {
		require.Fail(t, "Expected error when pushing without a valid registry")
	}
