...
>>>>>>> 785acec0b (extism)
	...
	"helm.sh/helm/v4/pkg/plugintypes"
)
...
func tplFun(parent *template.Template, includedNames map[string]int, strict bool) func(string, interface{}) (string, error) {
	return func(tpl string, vals interface{}) (string, error) {
		...
		t.Funcs(template.FuncMap{
			...
			"tpl":     tplFun(t, includedNames, strict),
=======
func NewChartRenderer(pc plugins.Catalog) (*ChartRenderer, error) {
	p, err := pc.GetPlugin(
		plugins.HookDescriptor{
			HookName:   "chart-renderer",
			APIVersion: "v0",
>>>>>>> 785acec0b (extism)
		})
<<<<<<< HEAD
...
=======
	if err != nil {
		return nil, fmt.Errorf("failed to get chart renderer plugin: %w", err)
>>>>>>> 785acec0b (extism)
	}
...
type pluginRenderEngine struct {
	plg plugins.PluginInstance
}

func (p *pluginRenderEngine) RenderChart(chrt *chart.Chart, renderValues chartutil.Values) (map[string]string, error) {

	renderValuesJSON, err := json.Marshal(renderValues)
	if err != nil {
		return nil, fmt.Errorf("failed to encode render values json: %w", err)
	}

	req := plugintypes.RendererPluginRenderChartRequestV0{
		Chart:            chrt,
		RenderValuesJSON: renderValuesJSON,
	}

	resp := plugintypes.RendererPluginRenderChartResponseV0{}
	if err := p.plg.Invoke(context.Background(), req, resp); err != nil {
		return nil, fmt.Errorf("chart renderer plugin error: %w", err)
	}

	result := make(map[string]string, len(resp.Manifests))
	for _, m := range resp.Manifests {
		result[m.Filename] = string(m.Content)
	}
